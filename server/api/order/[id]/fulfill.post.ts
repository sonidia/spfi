    ...
    for (const proxyUrl of proxyVariants) {
      ...
      try {
        ...
        try {
          ...
          const explicitFO = fulfillmentInfo?.line_items_by_fulfillment_order?.[0];
          if (explicitFO?.fulfillment_order_id) {
            ...
            const foRes = await axios.get(`${baseURL}/orders/${id}/fulfillment_orders.json`, commonAxiosConfig);
            const fulfillmentOrders = foRes.data.fulfillment_orders;
            openFO = fulfillmentOrders.find((fo: any) => fo.status === 'open' || fo.status === 'in_progress');
            
            if (openFO) {
              foLineItems = (openFO.line_items || []).map((li: any) => ({
                id: li.id,
          ...
          if (openFO) {
            ...
            if (foLineItems && foLineItems.length > 0) {
              fulfillmentPayload.line_items_by_fulfillment_order[0].fulfillment_order_line_items = foLineItems;
            }

            const createRes = await axios.post(`${baseURL}/fulfillments.json`, {
          ...
          console.warn("[Fulfillment] Modern way failed, trying legacy fallback...", foErr.message);
          // Fall through to legacy way below
        }

        // --- TRY LEGACY WAY (Orders/Fulfillments) ---
        // 1. Get locations (required for legacy fulfillment)
        const locRes = await axios.get(`${baseURL}/locations.json`, commonAxiosConfig);
        ...
        const legacyRes = await axios.post(`${baseURL}/orders/${id}/fulfillments.json`, {
          fulfillment: {
            ...
            tracking_company: fulfillmentInfo?.tracking_info?.company || "FedEx"
          }
        }, commonAxiosConfig);
