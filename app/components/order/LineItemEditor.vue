  ...
  <section class="line-editor" aria-labelledby="line-editor-title">
    ...
    <div v-if="isOpen && session" class="editor-body">
      <div v-if="session.hasMoreLineItems" class="panel-note">
        This order has more than 250 original lines. Shopify only exposes the first 250
        lines when an edit session begins, so additional lines are not available in this
        editor.
      </div>
      <div v-for="lineItem in session.lineItems" :key="lineItem.id" class="item-row">
        <div class="item-copy">
          ...
          <small>
            ...
            <template v-else> {{ lineItem.editableQuantity }} editable </template>
        ...
        <div class="item-controls">
          <label>
            ...
            <input
              ...
              step="1"
              :disabled="lineItem.editableQuantity <= 0 && lineItem.quantity > 0"
      ...
      <div class="custom-item-section">
        ...
        <div class="custom-item-grid">
          ...
          <label>
            <span>Quantity</span>
            <input v-model.number="customQuantity" type="number" min="1" step="1" />
          </label>
      ...
      <div class="editor-actions">
        ...
        >
          ...
          Commit {{ pendingChangeCount }} change{{
...
.custom-item-grid {
  ...
  grid-template-columns:
    minmax(180px, 2fr) minmax(120px, 1fr) 90px minmax(190px, 1fr)
    auto;
  align-items: end;
  gap: 10px;
}
.custom-item-grid > label {
  display: grid;
  gap: 4px;
}
.custom-flags {
...
.pending-custom-item > div {
  display: grid;
  min-width: 0;
}
.pending-custom-item strong {
  overflow: hidden;
  color: var(--text);
  font-size: 12px;
  text-overflow: ellipsis;
...
.panel-error {
  border-top-color: rgba(180, 49, 43, 0.2);
